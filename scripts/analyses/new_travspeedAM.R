...
library(lme4)
ModelPM<-lmer(speed ~ Storm2+(1|Device_ID),data=tdatAM,REML=T)
ranef(ModelPM)
...
######I THINK REVIEWER WOULD PREFER SE IF YOU ARE EXPRESSING AS SUCH*****???
##find CIs
...
library(ggplot2)
plot_speedAM<-ggplot(plotdata, aes(x=as.factor(Storm2), y=mean)) +
  ...
  scale_y_continuous(breaks=0:40*4,limit=c(12.55,29))+
...
##save
#ggsave(filename="scripts/snowfall/omit_closed/thesis-final/snowfall_AMtravel.png",dpi=800,units="cm",height=10,width=15)
